              ...
              return (
                <div key={index} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div className={`flex max-w-[70%] ${isMine ? "flex-row-reverse" : "flex-row"} items-end gap-2`}>
                    {/* Avatar */}
                    <div
                      className={`w-8 h-8 rounded-full shrink-0 ${isMine ? "bg-blue-100" : "bg-linear-to-br from-purple-400 to-purple-600"} flex items-center justify-center text-sm font-semibold ${
                          ...
                          <div className="mt-1">
                            {msg.file_url?.match(/\.(jpg|png|jpeg|gif)$/) ? (
                              ...
                              <a href={msg.file_url} target="_blank" className="text-blue-400 underline">
                                <i className="fas fa-file"></i> {msg.file_name}
                              </a>
                            )}
                          </div>
                        ) : msg.msg_type === "audio" ? (
